        ...
        <div class="sidebar-collapse">
            <ul class="nav" id="main-menu">
                ...
                <li><a href="/student/correct"><i class="fa fa-bar-chart-o"></i>
                    批改作业</a></li>
                <li><a href="/student/change"><i class="fa fa-pencil"></i>
                    修改密码</a></li>
                ...
                <div class="modal-body">
                    <form class="form-horizontal" role="form">
                        ...
                        <div class="form-group">
                            <label for="upload-file" class="col-sm-2 control-label">文件</label>
                            <div class="col-sm-10">
                                <input type="hidden" disabled="disabled" class="form-control" id="upload-id"  required="required">
                                <input type="file" accept="application/msword" class="form-control" id="upload-file"  required="required">
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    $(".panel-body").on('click', 'button#appeal', function () {
        ...
        {
            $.ajax({
                url:"/student/addAppeal",
                type:"post",
                data:{
                    'studentHomeworkId':data.id
                },
                async:false,
    ...
    $(".panel-body").on('click', 'button#reupload', function () {

        var data = $("#dataTables-example").DataTable().row($(this).parents("tr")).data();

        /*  alert("Do you want delete " + data.username + "?"); */
        var r=confirm("确认重传文件？");
        if (r==true)
        {
            document.getElementById('upload-id').value = data.id;
            document.getElementById('upload-file').value = "";
                ...
                $('#dataTables-example').dataTable( {
                    ...
                    "aoColumns": [
                        ...
                        {"mData" : function(obj){
                            ...
                            return obj.grade.toFixed(2);//通过调用函数来格式化所获取的时间戳
                        }},
                        {"mData" : function(obj){
                        ...
                        //},
                        {
                            ...
                            "render": function(data, type, row, meta) {
                                ...
                                if(row.grade != null) {
                                    if(row.status == -1){
                                        button += "<button style='margin-right: 10px;' id='appeal' class='btn btn-primary' type='button'>申诉</button>";
                                    }else if(row.status == 0){
                                        button += "<button style='margin-right: 10px;' disabled='disabled' class='btn btn-primary' type='button'>申诉中</button>";
                                    }else if(row.status == 1){
                                        button += "<button style='margin-right: 10px;' disabled='disabled' class='btn btn-primary' type='button'>已处理</button>";
                                    }else if(row.status == 2){
                                        button += "<button style='margin-right: 10px;' disabled='disabled' class='btn btn-primary' type='button'>已驳回</button>";
                                    }
